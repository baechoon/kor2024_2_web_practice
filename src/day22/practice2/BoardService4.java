    ...
    public static void main(String[] args) {
        ...
        while (true){
            ...
            if (choose==1){
                ...
                //기존 배열 내 게시물들을 새로운 배열에 이동하기 --> 배열복사
                if (count!=1){// 만약 기존 배열내 새로운 게시물이 존재하면
                    for (int index=0;index<boardList.length;index++){
                        newBoardList[index]=boardList[index];
                        // 기존 배열내 게시물들을 새로운 배열에 대입/복사
                    }//for e

                }//if e
                // 새로운 배열내 마지막 인덱스(배열명.length-1)에 입력받는 게시물 객체 등록
