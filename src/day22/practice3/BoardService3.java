...
//클래스: 멤버변수(핑드), 생성자,메서드(멤버함수)로 구성된 새로운 타입 만들기
        ...
        while (true){
            ...
            if (choose==1){
                ...
                for (int index=0;index<boardList.length;index++){
                    if (boardList[index]==null){// 만약에 인덱스번째 게시물이 비어있다면
                        Board board=new Board();//게시물 객체 생성
                        //생성된 게시물 객체 내 입력 받은 값을 대입한다
                        board.content=content;
                        board.writer=writer;
                        board.pwd=pwd;
                        //베열 내 index 번째 위치에 생성한 객체를 저장/ 대입한다
                ...
                for (int index=0;index<boardList.length;index++){
                    if (boardList[index]!=null){
                        System.out.printf("작성자:%s, 내용:%s",boardList[index].writer,boardList[index].content);
                    }//if e
                }//for e
